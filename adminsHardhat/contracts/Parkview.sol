...
contract SocietyBlock {
    ...
    address public govermentAuthority =
        0xd9145CCE52D386f254917e481eB44e9943F39138;
    address public landInspectorContract =
        0xd9145CCE52D386f254917e481eB44e9943F39138;
    ...
    event PrpertyTransactionRecord(
        uint indexed PropertyId,
        uint indexed BuyerCnic,
        uint indexed SellerCnic,
        uint SharesAmount,
        uint Prize,
        uint Time
    );
    ...
    function approve(
        ...
        uint _checkPropertiesAmount
    ) public isGovermentAuthority {
        require(
            currentCountProperties <= _checkPropertiesAmount,
            "Amount of total properties is grater"
        );
        require(address(this) == _blockAddress, "Wrong contract");
        isLdaApproved = true;
    }

    function increaseProperties(
        address _blockAddress,
        uint _addAmount
    ) public isGovermentAuthority {
    ...
    event AddNewPropertyLog(
        uint indexed PropertyId,
        address indexed AddByInspector,
        uint Time
    );
